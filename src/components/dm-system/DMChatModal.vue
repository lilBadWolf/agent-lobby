        ...
        <div v-if="currentTab" class="chat-section" :class="{ 'has-video-call': !!activeVideoCallUser, 'game-active': !!activeGame }">
          <div v-if="!activeGame && (chatToastNotices.length > 0 || outgoingRequestsForCurrentTab.length > 0)" class="notice-stack notice-stack-inline">
            ...
            >
              ...
              <div v-if="notice.type === 'audio-call' || notice.type === 'video-call' || notice.type === 'file-offer' || notice.type === 'pong-request' || notice.type === 'battleship-request' || notice.type === 'tictactoe-request' || notice.type === 'chess-request'" class="notice-buttons" :class="{ 'stack-like': notice.type === 'audio-call' || notice.type === 'video-call' }">
                <button class="btn-accept" :class="{ 'stack-like': notice.type === 'audio-call' || notice.type === 'video-call' }" @click="acceptNotice(notice)">{{ notice.type === 'audio-call' || notice.type === 'video-call' || notice.type === 'pong-request' || notice.type === 'battleship-request' || notice.type === 'tictactoe-request' || notice.type === 'chess-request' ? '✅' : 'ACCEPT' }}</button>
                <button class="btn-reject" :class="{ 'stack-like': notice.type === 'audio-call' || notice.type === 'video-call' }" @click="rejectNotice(notice)">{{ notice.type === 'audio-call' || notice.type === 'video-call' || notice.type === 'pong-request' || notice.type === 'battleship-request' || notice.type === 'tictactoe-request' || notice.type === 'chess-request' ? '❌' : 'REJECT' }}</button>
              </div>
            </div>
          ...
          <template v-else>
            <div v-if="activeGame" class="pong-game-container">
              ...
              />
              <BattleshipGame
                v-else-if="activeGame === 'battleship'"
              ...
              />
              <TicTacToeGame
                v-else-if="activeGame === 'tictactoe'"
                :key="`tictactoe-${activeGameUser || currentTab}-${getGameChat()?.dataChannel?.id ?? 'none'}`"
                :user="props.username"
                :peer-name="activeGameUser || currentTab"
                :data-channel="getGameChat()?.dataChannel ?? null"
                :start-signal="tictactoeStartSignal"
                :is-initiator="isGameInitiator"
                :waiting-for-acceptance="pendingGameRequest === 'tictactoe'"
                @close="closeActiveGame"
              />
              <ChessGame
                v-else
                :key="`chess-${activeGameUser || currentTab}-${getGameChat()?.dataChannel?.id ?? 'none'}`"
                :user="props.username"
                :peer-name="activeGameUser || currentTab"
                :data-channel="getGameChat()?.dataChannel ?? null"
                :start-signal="chessStartSignal"
                :is-initiator="isGameInitiator"
                :waiting-for-acceptance="pendingGameRequest === 'chess'"
                @close="closeActiveGame"
              />
            </div>
...
import PongGame from './dm-games/PongGame.vue';
import BattleshipGame from './dm-games/BattleshipGame.vue';
import TicTacToeGame from './dm-games/TicTacToeGame.vue';
import ChessGame from './dm-games/ChessGame.vue';
...
type DMGameType = 'pong' | 'battleship' | 'tictactoe' | 'chess';
...
const tictactoeStartSignal = ref(0);
const chessStartSignal = ref(0);
...
function launchSelectedGame() {
  ...
  if (selectedGame.value === 'battleship') {
    requestBattleship();
    return;
  }

  if (selectedGame.value === 'tictactoe') {
    requestTicTacToe();
    return;
  }
...
function requestTicTacToe() {
  if (!currentTab.value) return;
  activeGame.value = 'tictactoe';
  activeGameUser.value = currentTab.value;
  pendingGameRequest.value = 'tictactoe';
  isGameInitiator.value = true;
  tictactoeStartSignal.value = 0;
  emit('requestTicTacToe', currentTab.value);
}

function requestChess() {
  if (!currentTab.value) return;
  activeGame.value = 'chess';
  activeGameUser.value = currentTab.value;
  pendingGameRequest.value = 'chess';
  isGameInitiator.value = true;
  chessStartSignal.value = 0;
  emit('requestChess', currentTab.value);
}
...
function closeActiveGame() {
  ...
  if (gameToCancel && targetUser) {
    ...
    if (gameToCancel === 'tictactoe') {
      emit('cancelTicTacToe', targetUser);
    }

    if (gameToCancel === 'chess') {
      emit('cancelChess', targetUser);
    }
  }
...
const emit = defineEmits<{
  ...
  requestTicTacToe: [user: string];
  acceptTicTacToe: [user: string];
  rejectTicTacToe: [user: string];
  cancelTicTacToe: [user: string];
  requestChess: [user: string];
  acceptChess: [user: string];
  rejectChess: [user: string];
  cancelChess: [user: string];
...
const chatToastNotices = computed(() => {
  ...
  return props.notices.filter((notice) => {
    if (
      ...
      || notice.type === 'tictactoe-request'
      || notice.type === 'tictactoe-accept'
      || notice.type === 'tictactoe-reject'
      || notice.type === 'tictactoe-cancel'
      || notice.type === 'chess-request'
      || notice.type === 'chess-accept'
      || notice.type === 'chess-reject'
      || notice.type === 'chess-cancel'
    ) {
...
function acceptNotice(notice: DMNotice) {
  ...
  if (notice.type === 'tictactoe-request') {
    activeGame.value = 'tictactoe';
    activeGameUser.value = notice.from;
    pendingGameRequest.value = null;
    isGameInitiator.value = false;
    tictactoeStartSignal.value += 1;
    emit('acceptTicTacToe', notice.from);
    return;
  }

  if (notice.type === 'chess-request') {
    activeGame.value = 'chess';
    activeGameUser.value = notice.from;
    pendingGameRequest.value = null;
    isGameInitiator.value = false;
    chessStartSignal.value += 1;
    emit('acceptChess', notice.from);
    return;
  }
...
function rejectNotice(notice: DMNotice) {
  ...
  if (notice.type === 'tictactoe-request') {
    emit('rejectTicTacToe', notice.from);
    return;
  }

  if (notice.type === 'chess-request') {
    emit('rejectChess', notice.from);
    return;
  }
...
watch(
  ...
  (newNotices) => {
    ...
    if (isGameInitiator.value && pendingGameRequest.value === 'tictactoe' && matchingNotice.type === 'tictactoe-accept') {
      tictactoeStartSignal.value += 1;
      pendingGameRequest.value = null;
    }

    if (isGameInitiator.value && pendingGameRequest.value === 'chess' && matchingNotice.type === 'chess-accept') {
      chessStartSignal.value += 1;
      pendingGameRequest.value = null;
    }
    ...
    if (
      isGameInitiator.value
      && pendingGameRequest.value === 'tictactoe'
      && matchingNotice.type === 'tictactoe-reject'
    ) {
      pendingGameRequest.value = null;
      activeGame.value = null;
      activeGameUser.value = null;
    ...
    if (
      isGameInitiator.value
      && pendingGameRequest.value === 'chess'
      && matchingNotice.type === 'chess-reject'
    ) {
      pendingGameRequest.value = null;
      activeGame.value = null;
      activeGameUser.value = null;
    ...
    if (matchingNotice.type === 'tictactoe-cancel' && activeGame.value === 'tictactoe') {
      pendingGameRequest.value = null;
      activeGame.value = null;
      activeGameUser.value = null;
      isGameInitiator.value = false;
    ...
    if (matchingNotice.type === 'chess-cancel' && activeGame.value === 'chess') {
      pendingGameRequest.value = null;
      activeGame.value = null;
      activeGameUser.value = null;
      isGameInitiator.value = false;
...
.notice-item.tictactoe-request {
  border-color: #ffbe68;
  background: rgba(30, 17, 4, 0.95);
  color: #ffe3bf;
  box-shadow: 0 0 18px rgba(255, 186, 90, 0.22);
  text-shadow: 0 0 8px rgba(255, 214, 160, 0.35);
}

.notice-item.chess-request {
  border-color: #d1b9ff;
  background: rgba(24, 15, 36, 0.95);
  color: #efe6ff;
  box-shadow: 0 0 18px rgba(185, 150, 255, 0.22);
  text-shadow: 0 0 8px rgba(220, 198, 255, 0.36);
}
...
.notice-item.tictactoe-accept,
.notice-item.tictactoe-reject,
.notice-item.tictactoe-cancel {
  border-color: #ffc574;
  background: rgba(28, 18, 8, 0.95);
  color: #ffe8c7;
  box-shadow: 0 0 14px rgba(255, 188, 100, 0.22);
}

.notice-item.chess-accept,
.notice-item.chess-reject,
.notice-item.chess-cancel {
  border-color: #c7a4ff;
  background: rgba(24, 16, 32, 0.95);
  color: #f0e6ff;
  box-shadow: 0 0 14px rgba(188, 146, 255, 0.2);
}
