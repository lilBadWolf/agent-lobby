<template>
  <div class="dm-window-root">
    ...
    <DMChatModal
      ...
      @request-tic-tac-toe="handleRequestTicTacToe"
      @accept-tic-tac-toe="handleAcceptTicTacToe"
      @reject-tic-tac-toe="handleRejectTicTacToe"
      @cancel-tic-tac-toe="handleCancelTicTacToe"
      @request-chess="handleRequestChess"
      @accept-chess="handleAcceptChess"
      @reject-chess="handleRejectChess"
      @cancel-chess="handleCancelChess"
...
import DMChatModal from '../components/dm-system/DMChatModal.vue';
...
function handleRequestTicTacToe(user: string) {
  debugLog('handleRequestTicTacToe', { user });
  dmRuntime.value?.sendTicTacToeRequest(user);
}

function handleAcceptTicTacToe(user: string) {
  debugLog('handleAcceptTicTacToe', { user });
  dmRuntime.value?.sendTicTacToeAccept(user);
}

function handleRejectTicTacToe(user: string) {
  debugLog('handleRejectTicTacToe', { user });
  dmRuntime.value?.sendTicTacToeReject(user);
}

function handleCancelTicTacToe(user: string) {
  debugLog('handleCancelTicTacToe', { user });
  dmRuntime.value?.sendTicTacToeCancel(user);
}

function handleRequestChess(user: string) {
  debugLog('handleRequestChess', { user });
  dmRuntime.value?.sendChessRequest(user);
}

function handleAcceptChess(user: string) {
  debugLog('handleAcceptChess', { user });
  dmRuntime.value?.sendChessAccept(user);
}

function handleRejectChess(user: string) {
  debugLog('handleRejectChess', { user });
  dmRuntime.value?.sendChessReject(user);
}

function handleCancelChess(user: string) {
  debugLog('handleCancelChess', { user });
  dmRuntime.value?.sendChessCancel(user);
}
